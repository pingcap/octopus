...
// NewSqllogictest returns the SqllogictestCase.
func NewSqllogictest(cfg *config.Config) Case {
	return &SqllogictestCase{
		cfg:      &cfg.SerialSuite.Sqllogictest,
		host:     cfg.Host,
		port:     cfg.Port,
		user:     cfg.User,
...
func (s *SqllogictestCase) Initialize() error {
	return nil
}
...
func (s *SqllogictestCase) Execute(ctx context.Context) error {
	startTime := time.Now()
	...
	mdbs := createDatabases(taskCount, s.port, s.host, s.user, s.password, s.cfg.DBName)
	defer closeDatabases(mdbs)

	go addTasks(fileNames, taskChan)

	for i := 0; i < taskCount; i++ {
		go doProcess(doneChan, taskChan, resultChan, mdbs[i], i, s.cfg.SkipError)
	}

	go doWait(doneChan, resultChan, taskCount)
	if errCnt := doResult(resultChan, startTime); errCnt > 0 {
...
func createDatabases(num int, port int, host string, user string, password string, dbname string) []*sql.DB {
	mdbs := make([]*sql.DB, 0, num)
	for i := 0; i < num; i++ {
		dbstring := fmt.Sprintf("%s:%s@tcp(%s:%d)/%s", user, password, host, port, dbname)
...
func doProcess(doneChan chan struct{}, taskChan chan string, resultChan chan *result, mdb *sql.DB, runid int, skipError bool) {
...
// Return error count
func doResult(resultChan chan *result, startTime time.Time) int64 {
