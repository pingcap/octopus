...
var (
	...
	interval    = flag.Duration("interval", 5*time.Second, "the interval")
...
var (
	defaultVerifyTimeout = 1 * time.Hour
...
func main() {
	...
	if len(*metricAddr) > 0 {
		...
		go util.PushPrometheus("cross-table-bank", *metricAddr, defaultPushMetricsInterval)
	}
	cfg := Config {
