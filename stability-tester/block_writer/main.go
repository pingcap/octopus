...
func init() {
	...
	flag.IntVar(&concurrency, "concurrency", 200, "concurrencyof worker")
...
func main() {
	...
	blockWriter := NewBlockWriterCase(tables, concurrency)
	err = blockWriter.Initialize(ctx, db)
	if err != nil {
		log.Fatalf("initialize %s error %v", blockWriter, err)
	}

	err = blockWriter.Execute(ctx, db)
	if err != nil {
