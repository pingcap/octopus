...
import (
	...
	rl "github.com/pingcap/tidb-operator/pkg/util/election/resourcelock"
)
...
func (le *LeaderElector) acquire() {
	...
	wait.JitterUntil(func() {
		...
		if !succeeded {
			log.Debugf("failed to renew lease %v", desc)
			return
		}
		le.config.Lock.RecordEvent("became leader")
		log.Infof("sucessfully acquired lease %v", desc)
		close(stop)
...
func (le *LeaderElector) renew() {
	...
	wait.Until(func() {
		...
		desc := le.config.Lock.Describe()
		if err == nil {
			log.Debugf("succesfully renewed lease %v", desc)
			return
		}
		le.config.Lock.RecordEvent("stopped leading")
		log.Infof("failed to renew lease %v", desc)
		close(stop)
	}, 0, stop)
}
